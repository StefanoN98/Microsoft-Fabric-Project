...
## Design Philosophy: Multiple Transformation Engines

Rather than forcing every table through a single tool, this project deliberately uses different transformation engines depending on the nature of the data and the volume involved:

| Method | Volume | Complexity | Full Load | Incremental | Skill Required | FinOps |
|---|---|---|---|---|---|---|
| Notebook (PySpark) | High | High | Yes | Yes (MERGE/CDF) | Spark | CU on-demand |
| Dataflow Gen2 | Medium | Medium | Yes | Yes (Merge) | Low-code | Fixed CU |
| Pipeline + Notebook | High | High | Yes | Yes | Mixed | CU on-demand |
| T-SQL Warehouse | Medium | Medium | Yes | Yes (MERGE) | SQL | CU on-demand |
| Structured Streaming | High | High | No | Continuous | Spark | Always active |
| Shortcut + View | Any | Minimal | Yes | Yes | SQL | Zero cost |

In this project:

- PySpark Notebooks handle ERP tables and SQL Server-sourced tables (large volumes, need for audit logging and data quality profiling)
- Dataflow Gen2 handles CRM tables (smaller volume, benefits from low-code visual transformations)
- A dedicated Notebook handles the semi-structured product_catalog table (nested JSON flattening)

The reasoning behind this split is not "use whatever tool is available", but a deliberate cost/complexity trade-off: Spark notebooks are the right tool when audit logging, data quality profiling, and reusable functions across many similar tables are needed; a low-code Dataflow is a better fit when there are only two tables to clean, since spinning up a full Spark job for that would be overkill; and nested/semi-structured data structurally requires code (a low-code tool cannot flatten arbitrary nested JSON as cleanly as a few lines of PySpark).
