...
The process is driven by the centralized config_ingestion.csv configuration file, making the ingestion logic dynamic and avoiding hard-coded table names within the pipeline. In fact for the source teh filter activity evaluates the `SourceFolder` column using the following expression: 
...
In order to promote 'bronze_product_catalog' it is not possible to follow a standard copy activity approach because two of its columns are **nested/semi-structured**:
...
Neither Power BI nor a standard relational Gold model can meaningfully consume nested or array-typed columns, they need to become ordinary columns and rows.

A plain Copy data activity cannot flatten these structures or trim/clean the values inside them. A dedicated notebook (`NB_Silver_Clean_&_load_product_catalog`) handles this in two conceptual steps:

1. **Recursive cleaning**: a trim/empty-to-NULL logic used is applied working recursively, reaching inside the nested attribute structure and inside every element of the image list — a plain top-level cleaning pass would not touch anything nested.
