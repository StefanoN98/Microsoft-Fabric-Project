...
- Read and filter configuration file:
```spark
rows = (
    df_config
...
- Apply a function to get quality data information:
...
- Apply a recursive function to clean data:
...
- Write cleaned data to silver layer returning the following output:
```spark
...
- Save result in an audit log file: Every execution of this notebook writes its results (row counts, duration, success/failure status) into a shared `silver_audit_log` Delta table, in append mode. This means the audit log accumulates history across runs giving a full execution history over time. If any table fails during processing, the notebook still attempts all remaining tables before raising an explicit error at the end, so a single failure does not silently block the rest of the batch.
- 
---
