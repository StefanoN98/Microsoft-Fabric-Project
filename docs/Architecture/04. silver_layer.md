...
2. **Flattening**: the nested `attributes` structure is expanded into ordinary top-level columns, and the `images` list is exploded so that each URL becomes its own row, tagged with a position index (so the first/main image can still be distinguished from secondary ones).
