...
2. **A dedicated notebook is used instead**: [NB_load_web_logs](../../fabric_workspace/01.%20bronze_layer/NB_load_web_logs.ipynb) downloads the file directly via Python (`urllib.request`) into the Bronze Lakehouse `Files` area, bypassing the pipeline's Copy Activity entirely.
