...
The implementation is available in [`NB_Bronze_Shortcut`](../../fabric_workspace/bronze_layer/notebooks/NB_Bronze_Shortcut.py)  and requires the following identifiers to be resolved beforehand:

Workspace ID — identifies the target Fabric workspace.
Lakehouse ID — identifies the Bronze Lakehouse where the shortcut is created.
ADLS2 Connection ID — identifies the existing ADLS2 connection registered in Fabric.
