...
The configuration files is stored inside the LH_Bronze Lakehouse in the Files folder:
```text
GlobalRetail
│
└── Bronze Layer
...
For the complete JSON code: [`PL_INGEST.json`](../../fabric_workspace/01.%20bronze_layer/PL_INGEST.json)
...
All the files are dinamically copied using the relative url in the source module: `@concat(item().SourceFolder, '/', item().ObjectName)`

Also the destination is settled dynamic pasting the data into the LH_Bronze into the following dynamic path: `@item().DestinationFolder / @item().ObjectName`
...
2. **A dedicated notebook is used instead**: [`NB_load_web_logs`](../../fabric_workspace/01. bronze_layer/NB_load_web_logs.ipynb) downloads the file directly via Python (`urllib.request`) into the Bronze Lakehouse `Files` area, bypassing the pipeline's Copy Activity entirely.
...
The implementation is available here [`NB_Bronze_Shortcut`](../../fabric_workspace/bronze_layer/notebooks/NB_Bronze_Shortcut.py)

---

### Branch 3 — SQL Server

This branch retrieves transactional data from the on-premises OLTP database, via an on-premises gatewaythat bridges the local SQL Server instance with Fabric's cloud pipelines. Since this data is already structured and relational, it is written directly into Bronze `Tables/`, skipping the intermediate `Files/` landing step used by the other two sources; there is no benefit in landing already-structured relational data as a raw file first.
...
The 3 branches converge to an unique notebbok in the pipeline:
...
This notebook is intentionally the **only** place in Bronze where a transformation-like operation happens (renaming columns), and even this is treated as a structural necessity, not a business rule, infact the data values themselves are never touched.
